...
try
    ...
    else % for linux, DropAction==1 if no Modifier or control pressed
        java_robot(fh, {'key' 'click'});
        ctlDn = strcmp(get(fh, 'SelectionType'), 'alt') || ...
                any(strcmpi(fh.UserData, 'control'));
    end

    nii = get_nii(strtrim(evt.Data)); % strtrim needed for string
    if ctlDn, addOverlay(nii, fh);
...
if ~isempty(strfind(c, 'a copy')) % a copy
...
try
    ...
    nii = dicm2nii(fname, pwd, 'no_save');
end
%%
