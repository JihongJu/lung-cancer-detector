...
for i = 1:nRun
    ...
    if ~isfield(s, fldsCp{1}) % assumption: 1st instance is earliest
        try h{i}{1}.(fldsCp{1}) = [s.AcquisitionDate s.AcquisitionTime]; end
    end
end
...
%         h{i}{1}.RefAcqTimes = (t - min(t)) * 24 * 3600 * 1000; % day to ms
    ...
    if pf.save_json, try save_json(h{i}{1}, fname); end; end %#ok<*TRYNC>
    ...
    if isfield(nii.hdr, 'hdrTilt')
        ...
        nii_tool('save', nii, [fname '_Tilt' ext], rst3D); % save phase nii
    end
...
elseif strncmpi(s.Manufacturer, 'GE', 2)
    try phPos = s.ProtocolDataBlock.VIEWORDER==1; catch, end % 1 == bottom_up
elseif strncmpi(s.Manufacturer, 'Philips', 7)
...
a = diff(sort(ipp));
del = sum(a)/numel(a) * 0.01; % 1% arbituary
if nargin>1 && gantryTilt, del = del * 10; end % arbituary
nSL = sum(a > del) + 1;
...
%% Subfunction: return NumberOfImagesInMosaic in Siemens mosaic, or 1 otherwise.
